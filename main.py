...
while True:
    ...
    try:
        ...
        print(
            "Пароль має бути довжиною більше 6 символів, мати велику літеру, спеціальний символ(!@#$%^&*), цифру"
        )
        password = input("Введіть пароль: ")
        if user_login(login, password) if isLogin else user_register(login, password):
            log_event(
                f"Користувач {'залогінився' if isLogin else 'зареєструвався'}: {login}"
            )
            break
...
while True:
    ...
    if mode == None:
        ...
        while True:
            ...
            if leave_mode(action, "Вихід з модулю транзакцій!"):
                break
            elif action == 0:
                if check_any_obj(len(txs) == 0, "Додайте хоча-б одну транзакцію!"):
                    continue
                tx = inquirer.select(
                    message="Оберіть транзакцію для детальнішого просмотру: ",
                    choices=[
                        {
                            "name": f"{id} --- {from_ts_to_str(txs[id]['time'])} - {txs[id]['balance_change']} USD",
                            "value": id,
                        }
                        for id in txs.keys()
                    ]
...
Категорії: {''.join([f'\n- {x}' for x in txs[tx]['categories']])}
                ...
                if check_any_obj(
                    len(categories) == 2, "Спочатку додайте хоча-б одну категорію!"
                ):
                    continue
                log_event("Додавання транзакція.")
                amount = valid_float(
                    "К-сть валюти (>0 - якщо дохід, <0 - якщо витрати): "
                )
                ...
                while tx_categories == []:
                    ...
                    tx_categories = inquirer.checkbox(
                        ...
                        choices=list(
                            filter(lambda x: not x in ["дохід", "витрати"], categories)
                        ),
                    ).execute()
                note = input("Нотатка: ")
                tx_time, tx_timestamp = valid_time(
                    "Час транзакції (dd.mm.yyyy hh:mm): "
                )
                id, am = add_tx(amount, note, currency, tx_categories, tx_timestamp)
                print(
                    f"Транзакція успішно додана з id: {id}, {amount} {currency} = {am} USD у {tx_time}"
                )
                ...
                for upd in upds_act:
                    if upd == 0:
                        ...
                        print(
                            list(
                                filter(
                                    lambda x: not x in ["дохід", "витрати"], categories
                                )
                            )
                        )
                        print(
                            list(
                                filter(
                                    lambda x: not x in ["дохід", "витрати"],
                                    txs[tx]["categories"],
                                )
                            )
                        )
                        ...
                        new_time, new_timestamp = valid_time(
                            "Введіть новий час транзакції  (dd.mm.yyyy hh:mm): "
                        )
                ...
                while txs2del == []:
                    ...
                    txs2del = inquirer.checkbox(
                        ...
                        choices=[
                            {
                                "name": f"{id} - {txs[id]['balance_change']} USD",
                                "value": id,
                            }
                            for id in txs.keys()
                        ]
                ...
                if not None in txs2del:
                    print(del_txs(txs2del))
            elif action == 7:
                opts = []
                while opts == []:
                    opts = inquirer.checkbox(
                        ...
                        choices=[
                            {"name": "За часом", "value": 0},
                            {"name": "За категоріями", "value": 1},
                        ],
                    ).execute()
                ...
                for opt in opts:
                    if opt:
                        ...
                        if time_assistance_mode == 0:
                            while True:
                                selected_time_1, selected_timestamp_1 = valid_time(
                                    "Рахувати транзакції від (dd.mm.yyyy hh:mm): "
                                )
                                selected_time_2, selected_timestamp_2 = valid_time(
                                    "Рахувати транзакції до (dd.mm.yyyy hh:mm): "
                                )
                                if selected_timestamp_1 < selected_timestamp_2:
                                    break
                                else:
                                    print("Час до повинен бути більшим за час від!")
                        elif time_assistance_mode == 1:
                            selected_timestamp_1, selected_timestamp_2 = (
                                all_month_timestamp()
                            )
                ...
                txs_amount = valid_int(
                    "Введіть скільки треба останіх транзакцій показати: "
                )
                report = create_report(
                    selected_timestamp_1,
                    selected_timestamp_2,
                    txs_amount,
                    selected_categories,
                )
                if not report:
                ...
                if action == 6:
                    sum = sum_txs_by_time(
                        list(txs.values()), selected_timestamp_1, selected_timestamp_2
                    )
                else:
                    sum = sum_txs(list(txs.values()), selected_categories)
                print(
                    f"Сума всіх транзакцій{f' за категоріями' if selected_categories else ''}: {'+' if sum >= 0 else '-'}{sum} USD"
                )
    elif mode == 1:
        while True:
            action = inquirer.select(
            ...
            if leave_mode(action, "Вихід з модулю курсів валют!"):
                break
            elif action == 0:
                if len(exchange_rates) == 1:
                    print("Нічого дивитись! Додайте валюти!")
                for currency, ex_rate in exchange_rates.items():
                    (
                        print(f"{ex_rate} {currency} = 1 USD")
                        if currency != "USD"
                        else None
                    )
            elif action == 3:
                ...
                if check_any_obj(
                    len(exchange_rates) == 1, "Нічого видаляти! Додайте валюти!"
                ):
                    continue
                currencies = []
                while currencies == []:
                    ...
                    currencies = inquirer.checkbox(
                        ...
                        choices=list(
                            filter(lambda x: x != "USD", exchange_rates.keys())
                        ),
                    ).execute()
                ...
                if check_any_obj(
                    len(exchange_rates) == 1 and action != 1,
                    "Нічого замінювати! Додайте валюти!",
                ):
                    continue
                currency = (
                    input("Символ нової валюти: ").upper()
                    if action == 1
                ...
                price = valid_float(
                    f"Введіть курс нової валюти {currency} к USD: "
                    if action == 1
                    else f"Введіть новий курс {currency} к USD: "
                )
        ...
        while True:
            ...
            if leave_mode(action, "Вихід з модулю категорії!"):
                break
            elif action == 0:
                print("\nНаявні категорії:")
                ...
                if check_any_obj(
                    len(trackers) == 0, "Нічого дивитись! Додайте трекери к категоріям"
                ):
                    continue
                ...
                if isTracker:
                    budget_limit = valid_float(
                        f"Введіть ліміт бюджету у категорії {new_cat} у USD: "
                    )
                ...
                if check_any_obj(
                    len(categories) == 2, "Нічого видаляти! Додайте категорії"
                ):
                    continue
                ...
                if check_any_obj(
                    len(trackers) == 0, "Нічого змінювати! Додайте трекери к категоріям"
                ):
                    continue
                ...
                if check_any_obj(
                    len(trackers) == 0, "Нічого видаляти! Додайте трекери к категоріям"
                ):
                    continue
