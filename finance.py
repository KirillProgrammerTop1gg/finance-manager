...
def del_txs(ids: str) -> str:
    for id in ids:
        if txs.get(id, None) == None: return f'Транзакція {id} для видалення не знайдена'
        txs.pop(id)
...
def del_currency(currencies: str) -> str:
    for currency in currencies:
        if exchange_rates.get(currency, None) == None: return f'Валюта {currency} для видалення не знайдена'
