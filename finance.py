...
def add_transaction(balance_change: float, note: str, categories: Optional[List[str]] = None) -> str:
    id = str(uuid.uuid4())
...
y = add_transaction(10, 'Super ', ['turtle'])
x = add_transaction(-5, 'Test', ['b'])
x = add_transaction(36, 'Test', ['turtle', 'b'])
print(transactions)
# edit_transaction(x, balance_change = -9, note = 'fu')
# print(transactions)
print(sum_txs(list(transactions.values())))
print(sum_txs(list(transactions.values()),'g'))